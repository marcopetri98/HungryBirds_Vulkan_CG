...
namespace graphics
{
	...
	void GraphicsEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		// TODO: we are just using one vertex buffer
	...
	// TODO: vertex buffers should be created for each 3D object, not just for one
	...
	void GraphicsEngine::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
	{
		...
		{
			throw std::runtime_error(getErrorStr(Error::VULKAN_FAIL_CREATE_VERTEX_BUFFER));
		}
		...
		{
			throw std::runtime_error(getErrorStr(Error::VULKAN_FAIL_ALLOCATE_VERTEX_BUFFER_MEMORY));
		}
	...
	{
		// TODO: texture loading is hardcoded
		int texWidth, texHeight, texChannels;
		stbi_uc* pixels = stbi_load("textures/cc0image.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void GraphicsEngine::endSingleTimeCommands(VkCommandBuffer commandBuffer)
	{
		...
		// TODO: if we want to use multiple queue submissions we could use fences
	...
	void GraphicsEngine::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
	{
		...
		{
			throw std::invalid_argument(getErrorStr(Error::VULKAN_FAIL_DOESNT_SUPPORT_TRANSITION_LAYOUT));
		}
	...
	void GraphicsEngine::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
	{
		...
		// TODO: we are assuming that the image has been already transitioned 
